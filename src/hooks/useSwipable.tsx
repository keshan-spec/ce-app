...
export default function useSwipeableIndexes(tabs: number): ReturnValue {
    ...
    const excludedDivs = ['.exclude-swipe', '.splide', '.slide-in', '.embla']; // Add classNames or ids of the divs to exclude

    const isEventInExcludedDiv = (event: HandledEvents): boolean => {
        return excludedDivs.some(selector => {
    ...
    const move = useCallback(
        (data: SwipeEventData) => {
            if (isEventInExcludedDiv(data.event)) return;
